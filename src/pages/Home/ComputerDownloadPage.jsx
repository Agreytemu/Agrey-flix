...
import { 
  ArrowLeft, Download, Laptop, Copy, Check, 
  AlertTriangle, Video, Info, ShieldCheck, 
...
export default function ComputerDownloadPage() {
  ...
  if (loading) {
    return (
      <div id="computer-download-loading-wrapper" className="min-h-screen bg-[#07090e] flex items-center justify-center">
        <AgreyFlixLoader text="Initializing secure Computer download console..." />
      </div>
    );
  }
  ...
  if (error || !details) {
    return (
      <div id="computer-download-error-wrapper" className="min-h-screen bg-[#07090e] text-white flex flex-col items-center justify-center p-6 text-center">
  ...
  return (
    <div id="premium-computer-download-page" className="min-h-screen bg-[#07090e] text-zinc-100 font-sans pb-24 overflow-x-hidden relative">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 pt-8">
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-5">
          ...
          <span className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-400 font-bold uppercase tracking-wider">
            <Laptop className="w-3.5 h-3.5 text-blue-400" /> Dedicated Desktop Portal
          </span>
        </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-[#0b0e14]/65 border border-white/5 p-5 rounded-3xl backdrop-blur-md shadow-2xl space-y-5">
              <div className="relative aspect-[2/3] w-full rounded-2xl overflow-hidden bg-zinc-900/60 border border-white/15 shadow-lg">
                ...
                <div className="absolute bottom-3 left-3 bg-blue-600 text-white font-extrabold text-[10px] px-2.5 py-1 rounded-md tracking-wider uppercase shadow">
                  Desktop Optimized
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {type === 'tv' && (
                  <div className="p-3 bg-blue-950/20 border border-blue-500/10 rounded-xl">
                    ...
                    <span className="text-sm font-bold text-blue-400">Season {season}, Episode {episode}</span>
                  </div>
                )}
                ...
                <div className="pt-3 border-t border-white/5 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-blue-400 font-bold">Windows / macOS / Linux</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#0b0e14]/40 border border-white/5 p-5 rounded-2xl space-y-3">
              <span className="text-[10px] font-black uppercase tracking-wider text-zinc-500 block font-mono">Recommended Downloader</span>
              <div className="flex gap-3 text-xs text-zinc-400">
                <Download className="w-5 h-5 text-blue-400 shrink-0" />
                <div>
                  <span className="font-bold text-zinc-200 block">Internet Download Manager (IDM)</span>
                  <span className="text-[11px] text-zinc-500">Industry standard PC download manager supporting multi-threaded link grabs.</span>
                </div>
              </div>
            </div>
          </div>

          {/* Column 2: Computer Download Controls */}
          <div className="lg:col-span-8 space-y-6">
            
            <div className="bg-gradient-to-r from-blue-950/20 to-zinc-900/30 border border-blue-500/15 p-6 rounded-3xl">
              <span className="text-blue-400 text-[10px] font-black uppercase tracking-widest bg-blue-500/30 px-3 py-1 rounded-full border border-blue-500/10">
                Gigabit-Pipe Extraction
              </span>
              <h2 className="text-2xl font-black text-white tracking-widest uppercase mt-4 mb-2">
                Desktop PC & Mac Download Console
              </h2>
              <p className="text-xs text-zinc-400 leading-relaxed font-medium">
                Welcome to the high-speed desktop download center. This interface generates clean video file objects mapped directly to standard desktop down loaders (IDM, Free Download Manager, NeatDM) to maximize download speeds on high-speed ethernet or Wi-Fi lines.
              </p>
            </div>
            ...
            <div className="bg-[#0b0e14]/80 border border-white/5 p-6 rounded-3xl space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <div className="flex items-center gap-2.5">
                  <span className="text-xs font-black px-2 py-1 rounded bg-blue-600/10 text-blue-400 border border-blue-500/10 font-mono">01</span>
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-4 rounded-xl border transition-all duration-300 flex justify-between items-center ${
                        isSelected 
                          ? 'bg-blue-950/10 border-blue-500/50 shadow-lg shadow-blue-950/10' 
            ...
            <div className="bg-[#0b0e14]/80 border border-white/5 p-6 rounded-3xl space-y-5">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <div className="flex items-center gap-2.5">
                  <span className="text-xs font-black px-2 py-1 rounded bg-blue-600/10 text-blue-400 border border-blue-500/10 font-mono">02</span>
                  <h3 className="text-sm font-extrabold text-white uppercase tracking-wider">Acquire Stream Assets</h3>
                </div>
              </div>
              ...
              <div className="p-4 bg-zinc-950/40 border border-white/5 rounded-2xl space-y-2">
                ...
                <code className="text-xs font-mono font-bold text-blue-400 block break-all bg-zinc-900/80 p-2.5 rounded border border-white/5">
                  {videoFileName}
                </code>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                <button
                  ...
                  className="py-3.5 px-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-all shadow-lg flex items-center justify-center gap-1.5 text-center active:scale-98 cursor-pointer"
                >
                  <Laptop className="w-4 h-4 text-blue-200" /> Download IDM EF2 File
                </button>

                <a
                  href={streamUrl}
                  download={videoFileName}
                  className="py-3.5 px-4 bg-zinc-900 hover:bg-zinc-800 text-white border border-white/5 hover:border-white/10 text-xs font-black uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-1.5 text-center cursor-pointer"
                >
                  <Download className="w-4 h-4 text-zinc-400" /> Direct Browser Download
                </a>
                ...
                >
                  ...
                  {copiedLink ? 'Link Copied!' : 'Copy Direct Stream URL'}
                </button>
              </div>

              <div className="p-4 bg-zinc-950/70 border border-blue-500/20 rounded-2xl text-xs text-zinc-400 space-y-2.5 leading-relaxed">
                <div className="flex items-center gap-2 text-blue-300 font-extrabold uppercase tracking-wider text-[10px]">
                  <Info className="w-3.5 h-3.5" /> PC/Mac integration guidelines
                </div>
                <ul className="space-y-2 text-[11px] list-inside pl-1 text-zinc-300">
                  <li className="flex items-start gap-2">
                    <span className="text-blue-400 font-bold">1.</span>
                    <span><strong>IDM EF2 Import (Recommended):</strong> Click <strong className="text-white">Download IDM EF2 File</strong>. Open IDM on your computer, navigate to **Tasks &gt; Import &gt; From IDM export file (.ef2)**, choose the downloaded file, and start queue processing. This guarantees maximum speeds and zero resume failures!</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-blue-400 font-bold">2.</span>
                    <span><strong>Direct Browser Download:</strong> Tap the browser button to trigger browser-native download protocols.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-blue-400 font-bold">3.</span>
                    <span><strong>Other Download Clients:</strong> Copy the stream URL and paste it into clients like NeatDM, Free Download Manager, or aria2.</span>
                  </li>
                </ul>
              </div>
            </div>
